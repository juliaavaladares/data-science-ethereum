...
def feature_importance(model, text,plot=True):
    '''
    ...
    if plot:
        ...
        plt.savefig(figures_path+"feature_importance_"+text+".png")
        plt.close()
...
def plot_dct(dct,string):
    ...
    fig.savefig(figures_path+"decistion_tree"+string+".png")
    plt.close()
...
def cross_validation(X,y,model, text, importance = False, plot_decision_tree=False, X_new_dataset = None, y_new_dataset = None):
    ...
    if importance:
        ...
        feature_importance(results_metrics[0][1], 'No_balance')
        feature_importance(results_metrics[1][1], 'Undersample')
        feature_importance(results_metrics[2][1], 'Oversample')
        feature_importance(results_metrics[3][1], 'Smote')
